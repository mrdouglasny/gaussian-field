...
/-! ## Step 2: Schwartz reflection / translation are L²-isometric

`∫ (Θ g)(t)² dt = ∫ g(t)² dt`, by change of variables `t ↦ -t`.
`∫ (T_τ g)(t)² dt = ∫ g(t)² dt`, by change of variables `t ↦ t - τ`. -/
...
private theorem schwartzTranslation_integral_sq (τ : ℝ) (g : SchwartzMap ℝ ℝ) :
    ∫ t, (schwartzTranslation τ g) t ^ 2 = ∫ t, g t ^ 2 := by
  -- schwartzTranslation τ g t = g (t - τ) (definitionally).
  have h_eq : (fun t : ℝ => (schwartzTranslation τ g) t ^ 2) =
      (fun t : ℝ => g (t - τ) ^ 2) := by
    funext t; rfl
  rw [h_eq]
  exact MeasureTheory.integral_sub_right_eq_self (fun t => g t ^ 2) τ
...
Proof: `inner_self_eq_norm_sq_to_K` (real Hilbert) + `lp.inner_eq_tsum`
...
private theorem cylinderMassOperator_normSq_eq_sum_perMode
  ...
  -- Step A: ‖(cylinderMassOperator L mass hmass f)‖² = ⟪(cylinderMassOperator L mass hmass f), (cylinderMassOperator L mass hmass f)⟫_ℝ (real Hilbert space identity)
  rw [show ‖cylinderMassOperator L mass hmass f‖ ^ 2 =
      @inner ℝ ell2' _ (cylinderMassOperator L mass hmass f)
        (cylinderMassOperator L mass hmass f) from
  ...
  rw [show (∑' i : ℕ, (cylinderMassOperator L mass hmass f i : ℝ) ^ 2) =
      ∑' i : ℕ,
        (DyninMityaginSpace.coeff (E := SchwartzMap ℝ ℝ) (Nat.unpair i).2
          (resolventMultiplierCLM (resolventFreq_pos L mass hmass (Nat.unpair i).1)
            (ntpSliceSchwartz L (Nat.unpair i).1 f))) ^ 2 from by
    apply tsum_congr; intro n
    rw [show (cylinderMassOperator L mass hmass f n : ℝ) =
        (cylinderMassOperator L mass hmass f : ℕ → ℝ) n from rfl,
  ...
  have hF_summable : Summable F := by
    -- F p = ‖(cylinderMassOperator L mass hmass f)‖²-style summand → summable since lp 2 has finite norm.
    -- Strategy: F (Nat.pairEquiv.symm n) = ((cylinderMassOperator L mass hmass f) n)^2 (after formula),
    ...
    have h_lp_sum : Summable (fun n : ℕ => ‖((cylinderMassOperator L mass hmass f) : ℕ → ℝ) n‖ ^ (2 : ENNReal).toReal) := by
      have := lp.memℓp (cylinderMassOperator L mass hmass f)
      rwa [Memℓp, if_neg (by norm_num : (2 : ENNReal) ≠ 0),
           if_neg (by norm_num : (2 : ENNReal) ≠ ⊤)] at this
    have h_real : (fun n : ℕ => ‖((cylinderMassOperator L mass hmass f) : ℕ → ℝ) n‖ ^ (2 : ENNReal).toReal) =
        (fun n : ℕ => ((cylinderMassOperator L mass hmass f) n) ^ 2) := by
      funext n
      rw [show ((2 : ENNReal).toReal = 2) from by norm_num,
          Real.rpow_two, Real.norm_eq_abs, sq_abs]
    rw [h_real] at h_lp_sum
    -- After substituting cylinderMassOperator_formula, the ((cylinderMassOperator L mass hmass f) n)^2 expression
    -- equals F (Nat.pairEquiv.symm n).
    have h_eq : (fun n : ℕ => ((cylinderMassOperator L mass hmass f) n) ^ 2) = (fun n : ℕ => F (Nat.pairEquiv.symm n)) := by
      funext n
...
/-! ## Step 6: Concrete discharge — time translation

`schwartzTranslation τ` satisfies both hypotheses:
* commutes with `R_ω` (Fourier multipliers commute with translation,
  via `realFourierMultiplierCLM_translation_comm`).
* is L²-isometric (`schwartzTranslation_integral_sq`). -/

theorem cylinderMassOperator_timeTranslation_norm_eq_proved
    (mass : ℝ) (hmass : 0 < mass) (τ : ℝ) (f : CylinderTestFunction L) :
  ...
  show ‖cylinderMassOperator L mass hmass
        ...
        ‖cylinderMassOperator L mass hmass f‖
  apply cylinderMassOperator_norm_eq_of_temporal_action L mass hmass (schwartzTranslation τ)
  · -- R_ω commutes with schwartzTranslation (Fourier multiplier ↔ translation).
    intro ω hω g
    show resolventMultiplierCLM hω (schwartzTranslation τ g) =
         schwartzTranslation τ (resolventMultiplierCLM hω g)
    exact realFourierMultiplierCLM_translation_comm (resolventSymbol ω)
      (resolventSymbol_hasTemperateGrowth ω hω) τ g
  · -- schwartzTranslation is L²-isometric.
    exact schwartzTranslation_integral_sq τ
