...
# Pietsch Nuclear Spaces

Defines the Pietsch characterization of nuclear spaces and proves that the
Dynin-Mityagin characterization (`GaussianField.NuclearSpace`) implies it.

## Two notions of nuclearity

The `GaussianField.NuclearSpace` typeclass uses the **Dynin-Mityagin**
...
- `PietschNuclearSpace` — Pietsch characterization typeclass
- `exists_CLF_le_seminorm` — Hahn-Banach for continuous seminorms
- `NuclearSpace.toPietschNuclearSpace` — DM implies Pietsch
...
/-! ### Pietsch Nuclear Space Definition -/

/-- A locally convex topological vector space over ℝ is **Pietsch nuclear** if for every
...
class PietschNuclearSpace (E : Type*) [AddCommGroup E] [Module ℝ E]
...
gives rise to a `PietschNuclearSpace` (Pietsch nuclear dominance).
...
theorem NuclearSpace.toPietschNuclearSpace (E : Type*)
    [AddCommGroup E] [Module ℝ E]
    [TopologicalSpace E] [IsTopologicalAddGroup E] [ContinuousSMul ℝ E]
    [hN : NuclearSpace E] : PietschNuclearSpace E where
