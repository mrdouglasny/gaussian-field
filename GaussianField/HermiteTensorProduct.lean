...
**1 sorry**: `schwartzRapidDecayEquiv` — the topological isomorphism
`SchwartzMap D ℝ ≃L[ℝ] RapidDecaySeq` for general finite-dimensional D.
...
/-- The Schwartz space on any finite-dimensional real normed space is
topologically linearly isomorphic to the space of rapidly decreasing sequences.

**Status**: sorry — the mathematical proof for general D uses iterated 1D
Hermite isomorphisms via Fubini slicing. The 1D case is fully proved in
`SchwartzNuclear.Basis1D`. -/
noncomputable def schwartzRapidDecayEquiv (D : Type*)
    [NormedAddCommGroup D] [NormedSpace ℝ D] [FiniteDimensional ℝ D] :
    SchwartzMap D ℝ ≃L[ℝ] RapidDecaySeq := sorry
...
This replaces the 5 axioms in the original `Axioms.lean` with a single
sorry (the isomorphism `schwartzRapidDecayEquiv`). -/
