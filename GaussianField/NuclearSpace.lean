...
# Nuclear Space Typeclass

Defines the `NuclearSpace` typeclass for locally convex spaces admitting
...
The construction of Gaussian measures works for any `NuclearSpace E`,
not just Schwartz spaces.

## Main definitions

- `NuclearSpace E` — typeclass for nuclear Fréchet spaces with Schauder basis
- `NuclearSpace.expansion_H` — recovery of the Hilbert-space expansion
...
CLF, so the intrinsic `NuclearSpace.expansion` applies. -/
theorem NuclearSpace.expansion_H
    {H : Type*} [NormedAddCommGroup H] [InnerProductSpace ℝ H] [CompleteSpace H]
    (T : E →L[ℝ] H) (w : H) (f : E) :
    @inner ℝ H _ w (T f) =
    ∑' m, (NuclearSpace.coeff m f) * @inner ℝ H _ w (T (NuclearSpace.basis m)) := by
  have hφ := NuclearSpace.expansion ((innerSL ℝ w).comp T) f
